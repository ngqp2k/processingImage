...
def main(img_path):
    ...
    cv2.imshow("out seg", img_color)

if __name__ == "__main__":
    assert len(sys.argv) == 2, "loi tham so"
    input_path_image = sys.argv[1]

    assert os.path.isfile(input_path_image), "file khong ton tai"
    main(input_path_image);
    cv2.waitKey()